...
int iDeltaX = 0;        // start from the left corner
int iDeltaY = 0;        // start from the top corner

typedef struct ballinfo{
  int diameter;
  float x;      // left corner position
  float y;      // top corner position
  BOOL tx;      // move to the right
  BOOL ty;      // move to the top
  float sx;     // x speed
  float sy;     // y speed
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow) {
  ...
  hwnd = CreateWindow (szAppName, "WP Lab#4 example (animation)",
                      ...
                      450, 350, NULL, NULL, hInstance, NULL);
...
void InitialiseBalls (BallInfo* balls[BALLS_NUMBER], BOOL allocate_space){
  int i, j;
  srand ( time(NULL) );
...
void DrawBools (HDC hdc, BallInfo* balls[BALLS_NUMBER], int cxClient, int cyClient) {
  int i, j, diameter;
  float dif, distance;
  int iDeltaX, iDeltaY;
  BOOL bMoveTop = FALSE;
  ...
  for (i = 0; i < BALLS_NUMBER; i++) {
    ...
    for (j = 0; j < i; j++) {
      ...
      if (distance <= ((balls[i]->diameter + balls[j]->diameter)/2)){
        // jump if there is massive collision
